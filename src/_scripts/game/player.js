...
module.exports = function (Crafty,
  {WIDTH: WIDTH, HEIGHT: HEIGHT, MAX_SPEED: MAX_SPEED, BORDER: BORDER}) {
  ...
  //       this._callback = frame.bind(this)
  //     }
  //
  //     return this
  //   },
  ...
  Crafty.c('Player', {
    init: function () {
      ...
      this.bind('Change', function (changed) {
        if (changed.hasOwnProperty('score')) {
          Crafty.trigger('NewScore', changed.score)
        }
      })
      this.attr({
        score: 1000
      ...
      this.onHit('Energized', function (hitInfo) {
        ...
        // NOTE: Just to avoid some extra hits on the newly created Ghost
    ...
    _score: function () {
      ...
      Crafty('Energized').each(function () {
        // NOTE: Consider centers, not origin (offside to the top left) (done?)
        min.push(Math.hypot(
      ...
      if (score !== 0) {
        ...
        this.attr('score', this.score + score)
    ...
    regenerate: function (info, ghost) {
      ...
      this.attr('score', info.score + ghost.score)
      // this.
      return this
    }
  })
